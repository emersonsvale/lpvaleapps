...
<template>
  <div
    ref="containerRef"
    class="card-swap-container mx-auto relative perspective-[900px] overflow-visible"
    :style="{
  ...
  >
    <div
      ...
      :key="index"
      ref="cardRefs"
      class="card-swap-card absolute top-1/2 left-1/2 rounded-lg border border-white bg-black [transform-style:preserve-3d] [will-change:transform] [backface-visibility:hidden]"
      :style="{
        width: typeof width === 'number' ? `${width}px` : width,
        height: typeof height === 'number' ? `${height}px` : height
      }"
...
const { library: gsapLib, loadLibrary: loadGSAP } = useGSAP()
